...
globalThis.bytebeat = new class {
	...
	parseUrl() {
		...
		let songData;
		if (hash.startsWith('#v3b64') || hash.startsWith('#GFLJBeat3-') || hash.startsWith('#4')) {
			try {
				const hashString = hash.startsWith('#GFLJBeat3-') ? atob(hash.slice(2)) : atob(hash.slice(11)) : atob(hash.slice(6));
