...
globalThis.bytebeat = new class {
	...
	parseUrl() {
		...
		let songData;
		if (hash.startsWith('#4') || hash.startsWith('#v3b64') || hash.startsWith('#GFLJBeat3-')) {
			try {
				const hashString = hash.startsWith('#GFLJBeat3-') ? atob(hash.slice(11)) : atob(hash.slice(6)) : atob(hash.slice(2));
